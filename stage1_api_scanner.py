...
# Direct body .value access: $Request.Body.X.value — indicates LabelValue type
BODY_DIRECT_VALUE_RE = re.compile(r'\$[Rr]equest\.(?:Body|body)\.(\w+)\.value\b')
...
# Coalesce across locations: $Request.Query.X ?? ... ?? $Request.Body.X
# Detects the defensive fallback pattern so we can deduplicate params that appear
# in both query and body due to independent regex matches.
COALESCE_QUERY_BODY_RE = re.compile(
    r'\$[Rr]equest\.(?:Query|query)\.(\w+)'
    r'(?:\.value)?'
    r'\s*\?\?'
    r'(?:\s*\$[Rr]equest\.(?:Body|body)\.\1\.value\s*\?\?)?'
    r'\s*\$[Rr]equest\.(?:Body|body)\.\1\b'
)

# Reverse coalesce: $Request.Body.X ?? ... ?? $Request.Query.X
COALESCE_BODY_QUERY_RE = re.compile(
    r'\$[Rr]equest\.(?:Body|body)\.(\w+)'
    r'(?:\.value)?'
    r'\s*\?\?'
    r'(?:\s*\$[Rr]equest\.(?:Query|query)\.\1\.value\s*\?\?)?'
    r'\s*\$[Rr]equest\.(?:Query|query)\.\1\b'
)

# Parameters exempt from coalesce deduplication — kept in query regardless
COALESCE_QUERY_EXEMPT = {"tenantfilter"}

# Ternary cross-location: $Request.Query.X ? $Request.Query.X : $Request.Body.X
TERNARY_QUERY_BODY_RE = re.compile(
    r'\$[Rr]equest\.(?:Query|query)\.(\w+)\s*\?\s*'
    r'\$[Rr]equest\.(?:Query|query)\.\1\s*:\s*'
    r'\$[Rr]equest\.(?:Body|body)\.\1\b'
)

# Reverse ternary: $Request.Body.X ? $Request.Body.X : $Request.Query.X
TERNARY_BODY_QUERY_RE = re.compile(
    r'\$[Rr]equest\.(?:Body|body)\.(\w+)\s*\?\s*'
    r'\$[Rr]equest\.(?:Body|body)\.\1\s*:\s*'
    r'\$[Rr]equest\.(?:Query|query)\.\1\b'
)

# If/else cross-location: if ($Request.Body.X) { ...Body.X... } else { ...Query.X... }
IFELSE_BODY_QUERY_RE = re.compile(
    r'if\s*\(\s*\$[Rr]equest\.(?:Body|body)\.(\w+)\b.*?\)\s*\{[^}]*'
    r'\$[Rr]equest\.(?:Body|body)\.\1\b[^}]*\}\s*else\s*\{[^}]*'
    r'\$[Rr]equest\.(?:Query|query)\.\1\b',
    re.IGNORECASE | re.DOTALL
)

# Reverse if/else: if ($Request.Query.X) { ...Query.X... } else { ...Body.X... }
IFELSE_QUERY_BODY_RE = re.compile(
    r'if\s*\(\s*\$[Rr]equest\.(?:Query|query)\.(\w+)\b.*?\)\s*\{[^}]*'
    r'\$[Rr]equest\.(?:Query|query)\.\1\b[^}]*\}\s*else\s*\{[^}]*'
    r'\$[Rr]equest\.(?:Body|body)\.\1\b',
    re.IGNORECASE | re.DOTALL
)

# Combined condition: if ($Request.Query.X ... -or $Request.Body.X ...)
COMBINED_CONDITION_RE = re.compile(
    r'if\s*\([^)]*\$[Rr]equest\.(?:Query|query)\.(\w+)\b[^)]*'
    r'-or[^)]*'
    r'\$[Rr]equest\.(?:Body|body)\.\1\b',
    re.IGNORECASE
)

# Loose coalesce: $Request.Query.X <anything> ?? <anything> $Request.Body.X
# Catches operators (-split, -as [int]) and extra ?? intermediaries between operands
LOOSE_COALESCE_QR_RE = re.compile(
    r'\$[Rr]equest\.(?:Query|query)\.(\w+)\b'
    r'.+?\?\?'
    r'.+?\$[Rr]equest\.(?:Body|body)\.\1\b'
)

# Reverse loose coalesce
LOOSE_COALESCE_BQ_RE = re.compile(
    r'\$[Rr]equest\.(?:Body|body)\.(\w+)\b'
    r'.+?\?\?'
    r'.+?\$[Rr]equest\.(?:Query|query)\.\1\b'
)
...
def scan_endpoint(ps1_path: Path) -> dict | None:
    ...
    # ── LabelValue type annotation for direct body .value access ─────────
    value_accessed = {p.lower() for p in BODY_DIRECT_VALUE_RE.findall(content)
                      if p.lower() not in PARAM_NOISE}
    for p in body_direct:
        if p["name"].lower() in value_accessed:
            p["type"] = "LabelValue"

    # ── Coalesced param names (query/body overlap from ?? pattern) ────────
    coalesced_names = {
        p.lower()
    ...
    # ── Fallback cross-location param names (ternary, if/else, loose ??) ──
    fallback_names = {
        p.lower()
    ...
    # Compute full dedup set: original coalesce + fallback patterns
    all_dedup_names = coalesced_names | fallback_names

    # Detect all query/body overlap via set intersection (catches every syntax)
    all_body_names = {p["name"].lower() for p in body_params}
    overlap_names = {q.lower() for q in query_raw} & all_body_names

    # Remaining overlap not matched by any fallback regex → dual_access
    dual_access_names = overlap_names - all_dedup_names - COALESCE_QUERY_EXEMPT

    if all_dedup_names or dual_access_names:
        is_write_method = any(m.upper() in ("POST", "PUT", "PATCH", "DELETE") for m in http_methods)
        exempt = COALESCE_QUERY_EXEMPT
        non_exempt_dedup = all_dedup_names - exempt

        # Deduplicate fallback params (same logic as original coalesce)
        if non_exempt_dedup or (exempt & all_dedup_names):
            if is_write_method:
                # POST/PUT/PATCH/DELETE: non-exempt dedup params → body only
                query_params = [p for p in query_params if p["name"].lower() not in non_exempt_dedup]
                # Exempt params (tenantFilter) → query only
                body_params = [p for p in body_params if p["name"].lower() not in (exempt & all_dedup_names)]
            else:
                # GET: dedup params → query only
                body_params = [p for p in body_params if p["name"].lower() not in all_dedup_names]

        # Tag deduped params with source marker
        for p in (query_params + body_params):
            if p["name"].lower() in all_dedup_names:
                p["source"] = "ast_coalesced"

        # Tag dual-access params (kept in both locations)
        for p in (query_params + body_params):
    ...
    return {
        "endpoint": endpoint,
        "function": stem,
        "file": str(ps1_path.relative_to(API_REPO)).replace("\\", "/"),
