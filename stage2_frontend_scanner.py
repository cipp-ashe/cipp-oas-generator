...
# postUrl="/api/EndpointName" — JSX prop on any component.
# Matches CippFormPage, CippWizard*, and any future component using postUrl.
# Not anchored to a specific tag — multi-line JSX tags with nested `>` in
# embedded expressions (e.g. dialogIcon={<Icon />}) break tag-anchored patterns.
# Only string literals (single or double quotes) — variable props are
# silently skipped, same as computed URL names in the other patterns.
POST_URL_RE = re.compile(
    r'\bpostUrl\s*=\s*["\'](?P<url>/api/(?P<ep>[A-Za-z][A-Za-z0-9_]*))["\']',
)
...
def scan_file(jsx_path: Path) -> list[dict]:
    ...
    rel_path = str(jsx_path.relative_to(FRONTEND_REPO)).replace("\\", "/")
