...
def build_path_entry(ep: dict, verbose: bool = False) -> dict:
    ...
    # Only emit requestBody for non-GET passthru endpoints (GET can't have a body)
    passthru = ep.get("passthru_resolved")
    if passthru and passthru.get("variants") and "GET" not in methods:
    ...
    if request_body and not all(m.upper() == "GET" for m in methods):
