...
from config import PASSTHRU_ENDPOINTS, OUT_DIR, GRAPH_FIELD_TYPES, EXAMPLE_HINTS
...
def _resolve_variant(
    ...
    # Inject example values into response properties for doc tool rendering
    if response_schema:
        response_schema = _inject_response_examples(response_schema)
...
def _inject_response_examples(schema: dict) -> dict:
    """Inject example values into response schema properties from EXAMPLE_HINTS.
    Also generates a top-level example array/object for Redocly rendering."""
    import copy
    schema = copy.deepcopy(schema)

    def _add_examples(props: dict) -> None:
        for name, prop_schema in props.items():
            if "example" in prop_schema:
                continue
            example = EXAMPLE_HINTS.get(name.lower())
            if example is not None:
                prop_schema["example"] = example

    def _build_sample_object(props: dict) -> dict:
        """Build a sample object from property examples and type defaults."""
        sample = {}
        _type_defaults = {"string": "string", "boolean": True, "integer": 0, "number": 0.0}
        for name, prop_schema in props.items():
            if "example" in prop_schema:
                sample[name] = prop_schema["example"]
            elif "type" in prop_schema:
                sample[name] = _type_defaults.get(prop_schema["type"], "string")
        return sample

    # Find properties to inject into
    target = schema
    is_array = target.get("type") == "array" and "items" in target
    if is_array:
        target = target["items"]
    if "properties" in target:
        _add_examples(target["properties"])
        # Build top-level example for Redocly
        sample = _build_sample_object(target["properties"])
        if sample:
            if is_array:
...
def _merge_computed_fields(schema: dict | None, computed_fields: list[dict]) -> dict | None:
    ...
    if schema is None:
        ...
        for f in computed_fields:
            prop = {"type": f["type"]}
            example = EXAMPLE_HINTS.get(f["name"].lower())
            if example is not None:
                prop["example"] = example
            props[f["name"]] = prop
        return _inject_response_examples(
    ...
    for f in computed_fields:
        if f["name"] not in existing:
            ...
            if example is not None:
                prop["example"] = example
            existing[f["name"]] = prop

    return _inject_response_examples(schema)
